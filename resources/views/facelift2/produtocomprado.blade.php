...
$permissao = $revista[1]->collections;
...
// Validação: Se o conteúdo da revista foi retornado corretamente pelo Controller (que verifica a compra na API), libera o acesso.
if(isset($revista) && !empty($revista) && isset($revista[0]->id)){
    $modalConteudo = 'permitido';
} else {
    $modalConteudo = 'espacoParaAssinantes';
}
                        ...
                        @endif
                          ...
                          @if($capitulo->language == 'pt')
                            <?php
                              ...
                              $itensCapitulo = null;
                              foreach ($revista[1]->productItems as $objeto) {
                                  // @dd($revista);
                                  // @dd($objeto);
                              ...
                              @if($semImagem == 0)
                                  <article class="row bg-white border" style="border-radius: 0.75rem;padding-inline: 1rem;margin-top: 16px;margin-inline: unset;">
                                    <div class="col-md-6 revistaImagemArtigo d-block d-md-none" style="margin-top: 2.25rem; background: url( {{ $capitulo->cover }}) no-repeat center center !important;">
                                    </div>
                                    <div class="col-md-6" style="margin-block: 2.25rem; display: flex;flex-direction: column;justify-content: space-between;">
                                      <div>
                                        ...
                                        <p class="revistaImagemArtigop1">{{ __("messages.RevistaBladeAutores") }}:
                                            @if (!empty($itensCapitulo->authors))
                                                @foreach ($itensCapitulo->authors as $autor)
                                        ...
                                        <p class="revistaImagemArtigop2">{{__("messages.RevistaBladeKeywords")}}: @foreach ($itensCapitulo->keywords ?? [] as $keyword){{ $keywords->keyword }}. @endforeach</p>
                                      </div>

                                      <div>
                                      @if($modalConteudo == 'permitido')
                                          <button type="button" id="artigoCap{{ $capitulo->id }}" class="btn btn-primary btn-action-primary" style="background: #CA1D53;border: unset;" data-bs-toggle="modal" data-bs-target="#leiaCapitulo{{$capitulo->id}}">Ler Capítulo</button>
                                        @else
                                          <button type="button" class="btn btn-primary btn-action-primary" style="background: #CA1D53;border: unset;" data-bs-toggle="modal" data-bs-target="#{{ $modalConteudo }}"> Ler Capítulo</button>
                                       @endif  
                              ...
                              @else
                                  <article class="row shadowbox-article bg-white border" style="border-radius: 0.75rem;padding-inline: 1rem;margin-top: 16px;margin-inline: unset;">
                                      <div class="col-sm-12" style="margin-block: 2.25rem;">
                                          ...
                                          @endif
                                          @if(($modalConteudo == 'permitido') OR  ($capitulo->customerCourtesy == 1)) 
                                            <button type="button" id="artigoCap{{ $capitulo->id }}" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#leiaCapitulo{{$capitulo->id}}">Ler Capítulo</button>
                                          @else
                                              <button type="button" class="btn btn-primary btn-action-primary" style="background: #CA1D53;border: unset;" data-bs-toggle="modal" data-bs-target="#{{ $modalConteudo }}">Ler Capítulo</button>
                        ...
                        @endif
                          @if($capitulo->language == 'en')
                            <?php
                              ...
                              // Procurar o item correspondente no segundo array pelo ID
                              $itensCapitulo = null;
                              if (isset($revista[2]) && is_iterable($revista[2])) {
                                  foreach ($revista[2] as $objeto) {
                                      if ($objeto->id == $idArtigo) {
                                          $itensCapitulo = $objeto;
                                          break;
