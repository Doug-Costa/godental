...
foreach ($revista[0]->productItems as $key => $capitulo){
    if($capitulo->id == Request()->segment(4)){
              ...
              <div class="logo-modal">
                <img style="max-height:39px; margin: 20px auto;display:block"src="https://novo.dentalgo.com.br/imagens/logo_dentalgo_branco_site.png">
                  <div class="modal-title tituloModal">
                    <h5 class="modal-title" id="compraLerAssine" style="text-align:center; font-family:prompt;">{{__("messages.ModTopo")}}</h5>
                    ...
                    <a href="{{ route('cadastrar') }}" class="btn btn-danger" style="margin-right: 25px; text-align:center; margin-left:15px; font-family:prompt;"  alt="assinar" data-bs-toggle="modal" data-bs-target="#modalCadastro">{{__("messages.ModAssine")}}</a>
                    <a href="{{ route('logar') }}" style="item-align:center; font-family:prompt " class="btn btn-primary btn-assinante" data-bs-toggle="modal" data-bs-target="#modalLogin">{{__("messages.ModAssine2")}}</a>
                  </div>
                  <div class="row">                  
                    <div class="col-4 iconeVantagem"> <i class="fa-solid fa-book-open modal-icons"></i><h6 class="textoModal">{{__("messages.IconsBook")}}</h6></div>
                    <div class="col-4 iconeVantagem"><i class="fa-solid fa-house-laptop modal-icons"></i><h6 class="textoModal">{{__("messages.IconsHouse")}}</h6></div>
                    <div class="col-4 iconeVantagem"> <i class="fa-solid fa-calendar-days modal-icons"></i></i><h6 class="textoModal">{{__("messages.IconsCalendar")}}</h6></div>
                  </div>
              </div>
            </div>
            <div class="modal-body color-modalbody">
              <div class="row">                
                <div class="d-flex justify-content-center align-items-center">
                    <a href="{{ route('compraartigo') }}/{{ $revista[0]->id }}/{{$capitulo->id}}" class="btn btn-primary btn-assinante1 text-center">
                        <span class="d-inline-block align-middle">{{__("messages.ArtigoBladeComprarArtigo")}}</span>
                    </a>
                </div>
                <p class="modal-title" style="color:#fff; text-align:center; font-family:prompt; word-spacing: 1px; font-size:13px;">{{__("messages.ArtigoBladeModalCompra")}}</p>
                <p class="modal-title" style="color:#fff; text-align:center; font-family:prompt; word-spacing: 1px; font-size:13px;">S{{__("messages.ArtigoBladeModalCompra2")}}</p>
                <p class="saibamais-duvida2">{{__("messages.ArtigoBladeDuvida")}} <a href="#" class="saibamais-duvida">{{__("messages.ArtigoBladeSaibaMais")}}</a></p>
              </div>                      
            </div>             
          </div>
        </div>
...
<div class="container">
  <div class="row">
      ...
      <div class="col-12 col-lg-6 col-sm-4">
            ...
            $capitulos = $revista[0]->productItems;          
                foreach ($capitulos as $key => $capitulo2) {
                    if($capitulo2->id == Request()->segment(4)){
                ...
                foreach ($arrayLanguage as $key => $capitulo4) {
                  if ($capitulo4->id == Request()->segment(4)) {
                      $encontrado = true;
            ...
            <?php if ($encontrado && $anterior) : ?>
                <a href="{{ route('artigo') }}/{{ $revista[0]->id }}/{{ str_replace(' ', '-', $revista[0]->title) }}/{{$anterior}}/" class="btn btn-secondary btn-prox-artigo button-pagearticle">{{__("messages.ArtigoBladeAnterior")}}</a>
            <?php endif; ?>
            <?php if ($encontrado && $posterior) : ?>
                <a href="{{ route('artigo') }}/{{ $revista[0]->id }}/{{ str_replace(' ', '-', $revista[0]->title) }}/{{$posterior}}/" class="btn btn-secondary btn-prox-artigo button-pagearticle1">{{__("messages.ArtigoBladeProximo")}}</a>
            <?php endif; ?>
              </div>
            <div class="col">
            </div>
      ...
      <div class="col-12 col-lg-6 col-sm-8">
        ...
        <p class="lead text-paragraph textstyle-h4-2">{{ limita_caracteres(strip_tags( strip_tags($capitulo->brief) ), 600, false) }}</p>
        <p class="revistaImagemArtigop1 text-paragraph textstyle-h4-2">{{__("messages.RevistaBladeAutores")}}: @foreach ($itensCapitulo->authors as $key => $autor){{ $autor->name }}, @endforeach</p>
        @if($modalConteudo == 'permitido')        
          <div class="col mt-4">
            <button type="button" class="btn btn-secondary btn-lerArtigo button-pagearticle" data-bs-toggle="modal" data-bs-target="#leiaCapitulo{{$capitulo->id}}">{{__("messages.SearchBladeRead")}}</button>
              ...
              @if(isset($capitulo->data->liberado))
                ...
                @else
                  @if($modalConteudo == 'permitido')
                    <button type="button" class="btn btn-primary openModalBtn" data-audio-url="{{$capitulo->data->gotalk}}" data-bs-toggle="modal" data-bs-target="#gotalk">GoTalks</button>
                  @else
                    <button type="button" class="btn btn-primary openModalBtn" data-audio-url="{{$capitulo->data->gotalk}}" data-bs-toggle="modal" data-bs-target="#{{ $modalConteudo }}" style="background-color:dimgrey; border-color: dimgrey;">GoTalk &nbsp;&nbsp;<i class="fa-solid fa-lock"></i></button>
                  @endif
                @endif
              @else
                @if($modalConteudo == 'permitido')
        ...
        @else
            <div class="col mt-4">
            @if($modalConteudo == 'permitido')        
                <button type="button" id="artigoCap{{ $capitulo->id }}" class="btn btn-secondary btn-lerArtigo button-pagearticle" data-bs-toggle="modal" data-bs-target="#leiaCapitulo{{$capitulo->id}}">{{__("messages.SearchBladeRead")}}</button>
                @if(isset($capitulo->data->gotalk))
                  @if(isset($capitulo->data->liberado))
                    @if($capitulo->data->liberado == 'true')
                      <button type="button" class="btn btn-primary openModalBtn" data-audio-url="{{$capitulo->data->gotalk}}" data-bs-toggle="modal" data-bs-target="#gotalk">GoTalks</button>
              ...
              @if(session()->get('usuarioPlanoID') != 274)
                <a href="{{ $capitulo->content }}" target="_blank" class="botao-download">  
                  <i class="fa-solid fa-download"></i>
                </a>
              @endif
            @else
                <button type="button" class="btn btn-secondary btn-lerArtigo button-pagearticle" data-bs-toggle="modal" data-bs-target="#{{ $modalConteudo }}"> {{__("messages.SearchBladeRead")}}</button>
                @if(isset($capitulo->data->gotalk))
                  @if(isset($capitulo->data->liberado))
                    @if($capitulo->data->liberado == 'true')
                      <button type="button" class="btn btn-primary openModalBtn" data-audio-url="{{$capitulo->data->gotalk}}" data-bs-toggle="modal" data-bs-target="#gotalk">GoTalks</button>
      ...
      <div class="col-sm-12">
          ...
          <div class="tab-content" id="myTabContent">
            ...
            <div class="col-12 col-lg-10 col-sm-8">
                  <a class="nav-link p-1 article-content-options text-title text-title-1 mb-0" href="{{ route('artigo') }}/{{ $revista[0]->id }}/{{ str_replace(' ', '-', $revista[0]->title) }}/{{ $capitulo->id }}/{{ str_replace(' ', '-', $capitulo->title) }}">{{$capitulo->title}}</a>
                  <p class="lead text-paragraph">{{ limita_caracteres(strip_tags( strip_tags($capitulo->brief) ), 500, false) }}</p>
                  <a class="lerartigobotao" href="{{ route('artigo') }}/{{ $revista[0]->id }}/{{ str_replace(' ', '-', $revista[0]->title) }}/{{ $capitulo->id }}/{{ str_replace(' ', '-', $capitulo->title) }}">{{__("messages.ColecoesLeia")}}</a>
              </div>
        </div>         
                          @else
                          <div class="row boxbordas1">
